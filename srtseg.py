...
"""
    ...
    def from_videos(self, videos):
        """Restore SRTSeg from a plain video list
        Videos only needs to have local_path, media_url() and duration.
        """
        start = 0
        ...
        for index, video in enumerate(videos):
            ...
            seg.originalIndex = index
            seg.selected = True
            seg.path = video.media_url()
            seg.start = timedelta(seconds=start)
            seg.end = timedelta(seconds=start + duration)
            seg.duration = timedelta(seconds=duration)
            self.segments.append(seg)
            start += duration
        return self
